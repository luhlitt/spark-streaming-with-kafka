...
rawRedisServerStreamingDf = spark\
    .readStream \
    .format("kafka") \
...
rawRedisStreamingDf = rawRedisServerStreamingDf.selectExpr("cast(key as string) key", "cast(value as string) value")
...
rawRedisStreamingDf.withColumn("value", from_json("value", redisMessageSchema))\
    .select(col("value.*"))\
...
zsetEntriesDecodedDf = zsetEntriesEncodedDf.withColumn("customer", unbase64(zsetEntriesEncodedDf.encodedCustomer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords

zsetEntriesDecodedDf.withColumn("customer", from_json("customer", customerSchema))\
    .select(col("customer.*"))\
...
rawStediEventsStreamingDf = spark\
    .readStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092") \
...
# TO-DO: cast the value column in the streaming dataframe as a STRING 
rawStreamingDf = rawStediEventsStreamingDf.selectExpr("cast(key as string) key", "cast(value as string) value")
...
rawStreamingDf.withColumn("value", from_json("value", customerRiskSchema))\
    .select(col("value.*"))\
...
customerRisk = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr("""
        customer = email
"""))
